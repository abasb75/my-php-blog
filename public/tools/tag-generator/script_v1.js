...
var twiter_list = '';
var copy = null;
var description_chars = null;
var title_chars = null;
window.onload = function (){
    ...
    title_chars = document.getElementById('title_chars');
    description_chars = document.getElementById('description_chars');
    ...
    title.oninput = function(){setCharacters(this,70,title_chars);}
    description.oninput = function(){setCharacters(this,150,description_chars);}

}
function setCharacters(el,chars,label){
    label.innerHTML = 'Character : '+el.value.length+'/'+chars;
    if(el.value.length > chars){
        el.setAttribute('class','alert');
        label.setAttribute('class','subtitle_label labelalert');
    }else{
        label.setAttribute('class','subtitle_label');
...
function add_utf8(){
    ...
    meta_tags_list += '<meta http-equiv="X-UA-Compatible" content="IE=edge">\n';
}
function add_title(){
    if(!title) return;
