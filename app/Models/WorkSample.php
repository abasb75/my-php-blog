...
class WorkSample extends Model{
    ...
    static function getByPage($pageNumber){
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM `work_samples` ORDER BY `id` DESC LIMIT $startIndex,$numPerPage;";
        ...
        while($r = $res->fetch_assoc()){
            ...
            $r['timing'] = self::getTiming($r['created_at']);
            $r['th-image'] = Asset::image('project/'.$r['id'].'/th-'.$r['image']);
            $r['image'] = Asset::image('project/'.$r['id'].'/th-'.$r['image']);
            $posts[]=$r;
        }
    ...
    static function getById($id){
        $sql = "SELECT * FROM `work_samples` WHERE `id`=?;";
        $connect = self::connectDB();
        ...
        if($r and $result=$r->fetch_assoc() ){
            ...
            $result['timing'] = self::getTiming($result['created_at']);
            $result['th-image'] = Asset::image('project/'.$result['id'].'/th-'.$result['image']);
            $result['image'] = Asset::image('project/'.$result['id'].'/th-'.$result['image']);
